...
.DEFAULT_GOAL := help
.PHONY: test run release/prepare release/tag .check_git_clean help
...
test:   ## Run test suite
	@mvn -q --batch-mode --update-snapshots verify
...
release/prepare: .check_git_clean	## Bumps version and creates release branch (call with 'release/prepare version=<semver>')
	...
	@mvn versions:set-property -Dproperty=revision -DnewVersion=$(version:v%=%) -DgenerateBackupPoms=false
